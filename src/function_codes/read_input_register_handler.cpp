...
CPDU CReadInputRegisterHandler::handle(const CPDU& request, CRegisterMap& registers) {
    ...
    if (start_address + reg_quantity > registers.get_input_register_count()) {
    ...
    for (uint16_t i = 0; i < reg_quantity; i++) {
        uint16_t value = registers.read_input_register(start_address + i);
        response.push_back(value >> 8);
        response.push_back(value & 0xFF);
    }

    return CPDU(0x04, response);
}
